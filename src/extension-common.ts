...
// Debounce and version control for auto preview
let autoPreviewDebounceTimer: NodeJS.Timeout | null = null;
let autoPreviewVersion = 0;
const AUTO_PREVIEW_DEBOUNCE_MS = 150; // Debounce delay in milliseconds

/**
 * Preheat PreviewProvider for all workspace folders to reduce first-open delay.
 * This runs in the background and doesn't block plugin activation.
 */
async function preheatPreviewProviders(context: vscode.ExtensionContext) {
  const workspaceFolders = vscode.workspace.workspaceFolders ?? [];
  for (const folder of workspaceFolders) {
    try {
      // Initialize PreviewProvider in background (don't await to avoid blocking)
...
export async function initExtensionCommon(context: vscode.ExtensionContext) {
      ...
      if (automaticallyShowPreview) {
        ...
        if (!previewProvider.isPreviewOn(editor.document.uri)) {
          ...
          try {
            ...
            console.warn(
              '[MPE] Failed to lock preview group on startup:',
              error,
            );
          }
        }
      ...
      if (tabGroups.all.length > 1) {
        ...
        if (secondGroup && secondGroup.tabs.length === 0) {
          try {
            ...
            console.warn(
              '[MPE] Failed to cleanup empty group on startup:',
              error,
            );
          }
        }
  ...
  async function openPreviewToTheSide(uri?: vscode.Uri) {
    ...
    // Lock the preview group to prevent other tabs from opening in it
    await lockPreviewGroup(editor);
  }

  /**
   * Lock the preview editor group (column 2) to prevent other tabs from opening in it.
   * This ensures the preview stays isolated from regular editing tabs.
   */
...
`,
  ...
  */
  ...
  /**
   * Close preview when the source markdown file is closed.
   */
  context.subscriptions.push(
   ...
   * Close preview automatically if `closePreviewOnNonMarkdown` is on and switching to non-markdown.
   * Uses debounce and version control to handle rapid file switching.
   */
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor((editor) => {
      ...
      autoPreviewDebounceTimer = setTimeout(async () => {
        // Check if this operation is still valid (no newer events)
        ...
        if (editor && editor.document && editor.document.uri) {
          ...
          if (isMarkdownFile(editor.document)) {
            ...
            // Check version again before async operation
            if (currentVersion !== autoPreviewVersion) {
              return;
            }
            ...
            if (previewProvider.isPreviewOn(sourceUri)) {
              ...
              // Final version check before opening preview
              if (currentVersion !== autoPreviewVersion) {
                return;
              }
            ...
            if (closePreviewOnNonMarkdown) {
              ...
              // Check version after async operation
              if (currentVersion !== autoPreviewVersion) {
                return;
              }
            ...
            // Move editor to primary group if it somehow ended up in the preview group
            // But DON'T cleanup empty groups here - it interferes with editor switching
