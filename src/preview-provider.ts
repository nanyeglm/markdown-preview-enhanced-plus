...
export class PreviewProvider {
  ...
  /**
   * Close the current preview (for single preview mode) or close preview for a specific source URI.
   */
  public closeCurrentPreview(sourceUri?: Uri) {
    if (getPreviewMode() === PreviewMode.SinglePreview) {
      if (PreviewProvider.singlePreviewPanel) {
        PreviewProvider.singlePreviewPanel.dispose();
        PreviewProvider.singlePreviewPanel = null;
      ...
      if (previews) {
        previews.forEach((preview) => preview.dispose());
        this.previewMaps.delete(sourceUri.toString());
      }
    }
    // Clear pending update timeouts for this source
    if (sourceUri) {
      const timeout = this.updateTimeouts.get(sourceUri.toString());
      if (timeout) {
        clearTimeout(timeout);
        this.updateTimeouts.delete(sourceUri.toString());
      }
    }
  }

  /**
   * Check if any preview is currently open.
   */
  public hasAnyPreviewOpen(): boolean {
    if (getPreviewMode() === PreviewMode.SinglePreview) {
      return !!PreviewProvider.singlePreviewPanel;
    } else {
      return this.previewMaps.size > 0;
    }
  }
